...
if __name__ == '__main__':
    ...
    for t in range(num_batch):
        ...
        if t > 0:
            print(f'Test {t}')
            loss, acc = test_ensemble(data_loader, finetuned_classifier_list, weight_list, device)
        ...
        finetuned_classifier_list = []
        for i in range(len(finetuned_classifier_list)):
            finetuned_classifier_list.append(classifier_list[i])
            optimizer = optim.Adam(finetuned_classifier_list[i].parameters(), lr=lr, weight_decay=decay)
            for i in range(finetuned_epochs):
                train(t_data_loader, finetuned_classifier_list[i], optimizer, device)
        finetuned_classifier_list.append(best_F)
        ...
        if len(classifier_list) < num_classifiers:
            ...
            F_replace = classifier_list[0]
            Q_map = [[0 for j in range(len(classifier_list))] for i in range(len(classifier_list))]

            correct_status_list = []
            for i in range(len(classifier_list)):
                correct_status = get_correct_status(classifier_list[i], t_data_loader, device)
            ...
            for index in range(len(classifier_list[:-1])):
                div = diversity(Q_map, index)
                if div > max_div:
                    max_div = div
                    F_replace = classifier_list[index]
            classifier_list.remove(F_replace)
            
        # get weights
        weight_list = []
        mse_g = MSE_gamma(t_data_loader, classes, device)
        for classifier in finetuned_classifier_list[:-1]:
            mse_i = MSE_i(classifier, t_data_loader, device)
