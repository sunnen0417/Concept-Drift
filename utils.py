...
# Training dynamic
def train_dynamic(train_loader, DP, optimizer, device, location_loader=None):
    DP.to(device)
    DP.train()
    criterion = nn.KLDivLoss(reduction='batchmean')
    if location_loader == None:
        for data, target in train_loader:
            data, target = data.to(device), target.to(device)
            optimizer.zero_grad()
        ...
        for (data, target), location in zip(train_loader, location_loader):
            data, target, location = data.to(device), target.to(device), location[0].to(device)
            optimizer.zero_grad()
            output = nn.functional.log_softmax(DP(data, location), dim=1)
            loss = criterion(output, target)
            loss.backward()
            optimizer.step()

# Testing dynamic
def test_dynamic(test_loader, DP, device, return_softmax=False, location_loader=None):
    DP.to(device)
    DP.eval()
    criterion = nn.KLDivLoss(reduction='batchmean')
    if return_softmax:
    ...
    with torch.no_grad():
        ...
        if location_loader == None:
            ...
            for (data, target), location in zip(test_loader, location_loader):
                data, target, location = data.to(device), target.to(device), location[0].to(device)
                logits = DP(data, location)
                output = nn.functional.log_softmax(logits, dim=1)
                loss = criterion(output, target)
                total_loss += loss.item() * len(data)
                if return_softmax:
...
def predict_dynamic(test_loader, DP, device, location_loader=None):
    DP.to(device)
    DP.eval()
    softmax_log = []
    with torch.no_grad():
        if location_loader == None:
            ...
            for data, location in zip(test_loader, location_loader):
                data, location = data.to(device), location[0].to(device)
                prob = nn.functional.softmax(DP(data, location), dim=1).tolist()
                softmax_log += prob
