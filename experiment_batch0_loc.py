...
if __name__ == '__main__':
    ...
    if dim == 2 and classes == 2:
        draw_boundary = True
        img_dir = f'{args.dataset}_num_batch_{trainset.num_batch}_cls_t_dp_0_loc_{last_step_method}_seed_{seed}'
    ...
    # Dynamic predictor
    DP = DynamicPredictor(in_size=classes, d_model=classes*2, location=True, location_dim=dim, location_weight=0.5).to(device)
    ...
    for t in range(num_batch):
        ...
        if t > 0:
            ...
            softmax_dataset = SoftmaxDataset(data_softmax_history, baseset.data, mode='train')
        ...
        if last_step_method != 'none':            
            if t >= activate_dynamic_t:
                ...
                softmax_dataset = SoftmaxDataset(data_softmax_history, baseset.data, mode='test')
