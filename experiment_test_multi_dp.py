...
from utils import train, test, train_soft, test_soft, train_dynamic, test_dynamic, predict_dynamic, draw_decision_boundary, split_train_valid, bootstrap
...
def get_parser():
    ...
    parser.add_argument('-mds', '--max_dp_size', type=int, default=3)
...
if __name__ == '__main__':
    ...
    dp_list = []
    dp_opt_list = []
    for i in range(max_dp_size):
        dp_list.append(DynamicPredictor(in_size=classes, d_model=classes*2).to(device))
        dp_opt_list.append(optim.AdamW(dp_list[-1].parameters(), lr=d_lr, weight_decay=d_decay))
    ...
    for t in range(num_batch):
        ...
        if t > 0:
            ...
            vote = []
            for DP in dp_list:
                vote.append(predict_dynamic(softmax_data_loader, DP, device))
            pred_next_softmax = np.mean(np.array(vote), axis=0)
        ...
        if t > 0:    
            ...
            print(f'Train dynamic predictors {t}')
            for i in range(max_dp_size):
                print(f'Train DP {i}')
                bootstrap_log = bootstrap(softmax_log)
                softmax_dataset = SoftmaxDataset(bootstrap_log, mode='train')
                ...
                for j in range(d_epochs):
                    print('Epoch:', j+1)
                    train_dynamic(t_softmax_data_loader, dp_list[i], dp_opt_list[i], device)
                    loss = test_dynamic(t_softmax_data_loader, dp_list[i], device)
                    print(f'Train loss:{loss}')
                    loss = test_dynamic(v_softmax_data_loader, dp_list[i], device)
                    print(f'Valid loss:{loss}')
                    if loss < best_loss:
                        best_loss = loss
                        best_DP = copy.deepcopy(dp_list[i])
                        best_opt_state_dict = copy.deepcopy(dp_opt_list[i].state_dict())
                        p = patience
                    else:
                        p -= 1
                        if p < 0:
                            ...
                            break
                dp_list[i] = best_DP
                dp_opt_list[i] = optim.AdamW(dp_list[i].parameters(), lr=d_lr, weight_decay=d_decay)
                dp_opt_list[i].load_state_dict(best_opt_state_dict)
