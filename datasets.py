...
set_seed(0)
dataset_dict = {
                'translate':TranslateDataset(), # Translation
                'rotate':RotateDataset(), # Rotation
                'ellipse':HyperballDataset(), # Ellipse
                'revolution':RevolutionDataset(), # Revolution
                'progress':ForwardDataset(), # Progress
                'admission':admission_dataset(k=10, normalize=True), # Graduate Admission
                'sale':house_sale_dataset(k=10, normalize=True), # House Sale
                'house':california_housing_dataset(num_batch=11, normalize=True), # California Housing
                'wine_white':wine_quality_dataset(dataset='white', normalize=True), # Wine White
                'wine_red':wine_quality_dataset(dataset='red', normalize=True), # Wine Red
                'power':CCPP_dataset(k=10, normalize=True), # Power Plant
                'price':ford_price_dataset(time_slice=10, normalize=True), # Ford Price
                'gas':GasSensorDataset(normalize=True), #Gas Sensor
                'covertype':CovertypeDataset(normalize=True), # Covertype
                'airline': AirlineDataset(normalize=True), # Airline
                'weather': WeatherDataset(normalize=True), # Weather
                'spam': SpamDataset(normalize=False), # Spam. The dataset has already been encoded as 0/1 features
                'kdd':KDD99Dataset(normalize=True), # KDD
                'electricity':ElectricityDataset(normalize=False), # Electricity. The dataset has already been normalized originally
                'onp':ONPDataset(normalize=True) # ONP (Online News Popularity)
               } 
