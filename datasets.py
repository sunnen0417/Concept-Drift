...
class GasSensorDataset(Data.Dataset):
    ...
    def set_t(self, t):
        ...
        if self.t == 0:
            ...
            self.batch0_len = len(self.target) 
            
    ##### only for baseset
    def concat_dataset(self, dataset): 
        self.all_data = np.concatenate((self.all_data, dataset.data))
        self.all_target = np.concatenate((self.all_target, dataset.target))
        
        # sampling
        #sample_indexes = random.sample(range(self.all_data.shape[0]), self.batch0_len)
        #self.data = self.all_data[sample_indexes,:]
        #self.target = self.all_target[sample_indexes]
        #print('sample index', sample_indexes[:10])
        
        self.data = self.all_data
        self.target = self.all_target
