...
def get_parser():
    ...
    parser.add_argument('-lsm', '--last_step_method', type=str, default='hard') # 'none', 'hard', 'soft', 'cost'
    ...
    parser.add_argument('-l', '--location', type=bool, default=True)
    parser.add_argument('-lw', '--location_weight', type=float, default=0.1)
    return parser
...
if __name__ == '__main__':
    ...
    if dim == 2 and classes == 2:
        draw_boundary = True
        img_dir = f'{args.dataset}_num_batch_{trainset.num_batch}_cls_t_dp_0_{last_step_method}_seed_{seed}_lw{args.location_weight}'
    ...
    for t in range(num_batch):
        ...
        if t > 0:
            ...
            base_tset, base_vset = split_train_valid(baseset, train_ratio=train_ratio)
            ...
            v_softmax_data_loader = Data.DataLoader(vset, batch_size=batch_size, 
                                                  shuffle=False, num_workers=num_workers)
            t_data_loader = Data.DataLoader(base_tset, batch_size=batch_size,
                                                shuffle=False, num_workers=num_workers)
            v_data_loader = Data.DataLoader(base_vset, batch_size=batch_size,
                                                shuffle=False, num_workers=num_workers)
            best_loss = float('inf')
            best_DP = None
            best_opt_state_dict = None
            ...
            for i in range(d_epochs):
                print('Epoch:', i+1)
                train_dynamic(t_softmax_data_loader, DP, d_optimizer, device, location_loader=t_data_loader)
                loss = test_dynamic(t_softmax_data_loader, DP, device, location_loader=t_data_loader)
                print(f'Train loss:{loss}')
                loss = test_dynamic(v_softmax_data_loader, DP, device, location_loader=v_data_loader)
        ...
        if last_step_method != 'none':            
            if t >= activate_dynamic_t:
                ...
                softmax_data_loader = Data.DataLoader(softmax_dataset, batch_size=batch_size, 
                                                      shuffle=False, num_workers=num_workers)
                pred_next_softmax = predict_dynamic(softmax_data_loader, DP, device, location_loader=baseset)
                ...
                if last_step_method == 'soft':
                    ...
                    tset, vset = split_train_valid(soft_baseset, train_ratio=train_ratio)
                    t_data_loader = Data.DataLoader(tset, batch_size=batch_size, 
                                                  shuffle=True, num_workers=num_workers)
                    v_data_loader = Data.DataLoader(vset, batch_size=batch_size, 
                                                  shuffle=False, num_workers=num_workers)
                    ...
                    baseset.target = y
                    tset, vset = split_train_valid(baseset, train_ratio=train_ratio)
                    t_data_loader = Data.DataLoader(tset, batch_size=batch_size, 
                                                  shuffle=True, num_workers=num_workers)
                    v_data_loader = Data.DataLoader(vset, batch_size=batch_size, 
                                                  shuffle=False, num_workers=num_workers)
